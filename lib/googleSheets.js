...
const getActiveListings = (allListings) => {
  return allListings
    .filter((item) => {
      ...
      // 2. Broken link/image check (Optimization from Lighthouse audit)
      if (isBrokenLink(item.image_url || item.Zdjecie)) return false;
      if (isBrokenLink(item.companyDetails?.WWW || item.WWW)) return false;
