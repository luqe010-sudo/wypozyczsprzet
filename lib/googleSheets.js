...
const assignListingSlugs = (activeListings) => {
  const slugCount = {};
  activeListings.forEach((item) => {
    let slug = generateCleanSlug(item['SprzÄ™t']);
    if (slugCount[slug]) {
      slugCount[slug]++;
    ...
    item.slug = slug;
    item.legacySlug = generateLegacySlug(item['SprzÄ™t'], item.Miasto);
  });

  const legacySlugCount = {};
  activeListings.forEach((item) => {
    let ls = generateLegacySlug(item['SprzÄ™t'], item.Miasto);
  ...
  try {
    ...
    if (!equipError && equipment && equipment.length > 0) {
      const normalized = equipment.map(item => {
        ...
        // Prefer branch data over company data for location
        const branch = item.company_branches || {};
        const branchCity = branch.city || company.city;
        const branchAddress = branch.address || company.address;
        const branchLat = branch.lat ?? company.lat;
        const branchLng = branch.lng ?? company.lng;
        const branchZip = branch.zip_code || company.zip_code;
        const branchPhone = branch.phone || company.phone;
        const branchEmail = branch.email || company.email;
        
        // Prefer category from FK, fallback to text field
        const catFromDb = item.equipment_categories;
        const rawCategory = catFromDb?.db_key || (item.category || '').trim();
        const Kategoria = catFromDb?.name || CATEGORY_DISPLAY_MAP[rawCategory] || rawCategory;
        const seoCategory = catFromDb?.slug || getSeoSlugByDbKey(rawCategory);

        // Prefer subcategory from FK, fallback to text field
        const subFromDb = item.equipment_subcategories;
        const subcategorySlug = subFromDb?.slug || item.subcategory || '';
        ...
        return {
          ...
          Miasto: branchCity,
          citySlug: getCitySlug(branchCity),
          Lokalizacja: branchAddress,
      ...
      const active = assignListingSlugs(getActiveListings(normalized));
...
const fetchSlugDataFromSupabase = async () => {
  const { data: equipment, error } = await supabase
    ...
    .select(`
      ...
      updated_at,
      companies (city),
      company_branches (city),
      equipment_categories (slug, db_key)
    `);
  ...
  const normalized = equipment.map((item) => {
    const branchCity = item.company_branches?.city || item.companies?.city || '';
    const rawCategory = item.equipment_categories?.db_key || (item.category || '').trim();

    return {
      ['SprzÄ™t']: item.name,
      Miasto: branchCity,
      citySlug: getCitySlug(branchCity),
      seoCategory: item.equipment_categories?.slug || getSeoSlugByDbKey(rawCategory),
      Status: item.status || 'active',
      priority: item.priority || 1,
      created_at: item.created_at,
      updated_at: item.updated_at,
  ...
  return assignListingSlugs(getActiveListings(normalized)).map((item) => ({
    slug: item.slug,
    legacySlug: item.legacySlug,
    seoCategory: item.seoCategory,
    citySlug: item.citySlug,
    cityName: item.Miasto,
    created_at: item.created_at,
    updated_at: item.updated_at,
  }));
};
...
const fetchListingSlugData = unstable_cache(
  async () => {
    try {
      const slugData = await fetchSlugDataFromSupabase();
      if (slugData) return slugData;
    } catch (error) {
      console.error('[Sitemap] Lightweight slug fetch failed, falling back to marketplace cache:', error);
    }
  ...
  ['listing-slugs'],
  { revalidate: 3600, tags: ['listings', 'listing-slugs'] }
);
