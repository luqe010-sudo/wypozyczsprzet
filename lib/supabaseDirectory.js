 ...
 */
async function fetchAllCompaniesUncached() {
...
export const fetchAllCompanies = unstable_cache(
  fetchAllCompaniesUncached,
  ['directory-companies'],
...
export async function getDirectoryIndexData() {
  const companies = await fetchAllCompanies();
  const cityMap = new Map();
  const voivMap = new Map();
  ...
  return {
    ...
    voivodeships: Array.from(voivMap.values()),
  };
}
