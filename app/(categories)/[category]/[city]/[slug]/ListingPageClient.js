...
export default function ListingPageClient({ listing, seoDescription, faqItems, related, categorySlug, categoryName }) {
  ...
  // Build the new-format URL for this listing
  const listingUrl = `/${listing.seoCategory}/${listing.citySlug}/${listing.slug}`;
  ...
  useEffect(() => {
    trackEvent('view_listing', {
      listing_name: name,
      listing_category: categoryName,
      listing_city: listing.Miasto,
    });
  }, [name, categoryName, listing.Miasto]);

  const handleClaimClick = () => {
    if (!user) {
      router.push(`/login?returnTo=${listingUrl}`);
      return;
    }
    ...
    <>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-12">
        <div className="lg:col-span-2">
          <Link
            href={`/${categorySlug}`}
            className="inline-block bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider mb-4 hover:bg-blue-200 dark:hover:bg-blue-900/50 transition-colors"
          >
            {categoryName}
          </Link>
      ...
      {related.length > 0 && (
        <section className="mb-12">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {related.map((item) => (
              <Link key={item.slug} href={`/${item.seoCategory}/${item.citySlug}/${item.slug}`} className="group no-underline">
      ...
      <div className="text-center mt-12 mb-8">
        <Link href={`/${categorySlug}`} className="inline-flex items-center gap-2 text-gray-600 dark:text-gray-400 hover:text-blue-600 dark:hover:text-blue-400 font-bold transition-colors">
          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 19l-7-7m0 0l7-7m-7 7h18" /></svg>
          Powrót do kategorii: {categoryName}
        </Link>
      </div>
