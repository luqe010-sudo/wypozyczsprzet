      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 shadow-sm">
        ...
        <form action={handleSubmit} className="p-6 space-y-6">
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
            ...
            <div className="sm:col-span-2">
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Właściciel (Użytkownik)</label>
              <CustomSelect
                options={users.map(u => ({ value: u.id, label: `${u.id.substring(0, 8)}... (${u.role})` }))}
                value={ownerUserId}
